...
<% else %>
		...
		<% for i=1 to TypeList_result_num step 1%>
			<% if len(TypeList_rs("c_code"))=3 then %>
				<BR>
				<a href="../product/product_List.asp?p_class=<%=TypeList_rs("c_code")%>" title="单击查看<%=TypeList_rs("c_Name")%>&nbsp;(<%=TypeList_rs("c_Code")%>)下级分类商品" class="CSS-Text1"> 
			  <img src="../images/sanjiao.gif" width="7" height="10" border="0">&nbsp;
			  <%=TypeList_rs("c_Name")%>&nbsp;&nbsp;&nbsp;</a>
			<% else%>
			  <a href="../product/product_List.asp?p_class=<%=TypeList_rs("c_code")%>" title="单击查看<%=TypeList_rs("c_Name")%>&nbsp;(<%=TypeList_rs("c_Code")%>)下级分类商品"> 
			  <img src="../images/sanjiao.gif" width="7" height="10" border="0">&nbsp;
			  <%=TypeList_rs("c_Name")%>&nbsp;&nbsp;&nbsp;</a>     
		<%  end if%>
			</TD><td>
