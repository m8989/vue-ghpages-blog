...
<% else %>
		...
		<tr> 
			...
			<% if len(TypeList_rs("c_code"))=3 then %>
				<!--<a href="eProduct.asp?p_class=<%=TypeList_rs("c_code")%>" title="单击查看<%=TypeList_rs("c_Name")%>&nbsp;(<%=TypeList_rs("c_Code")%>)下级分类商品" class="CSS-Text1"> 
			  <img src="../images/+.gif" width="7" height="10" border="0">&nbsp;
			  <%=TypeList_rs("c_Name")%>&nbsp;&nbsp;&nbsp;</a>--><!--阿政编写的注释:前面被注释的代码是去掉有阴影特效的文字"类别"!-->
			<% else%>
			  <a href="../product/e_Product_list.asp?p_class=<%=TypeList_rs("c_code")%>" title="单击查看<%=TypeList_rs("c_Name")%>&nbsp;(<%=TypeList_rs("c_Code")%>)下级分类商品"> 
			  <!--<img src="../images/+.gif" width="7" height="10" border="0">&nbsp;--><!--阿政编写的注释:前面被注释的代码是去掉一张图片!-->
