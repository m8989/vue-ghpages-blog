...
'进入页面时(非提交)
	...
	if editid<>"" then '编辑状态
		...
		sql="select * from " & table & " where newsID =" & editid
		'测试
...
function showUploadDialog(s_Type, s_Link, s_Thumbnail){
	...
	var arr = showModalDialog("../HtmlEditor/dialog/i_upload.htm?style=coolblue&type="+s_Type+"&link="+s_Link+"&thumbnail="+s_Thumbnail, window, "dialogWidth:0px;dialogHeight:0px;help:no;scroll:no;status:no");
}
function eWebEditorPopUp(form, field, width, height) {
	window.open("../HtmlEditor/popup.htm?style=popup&form="+form+"&field="+field, "", "width="+width+",height="+height+",toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no");
}
</script>
<% end if '************************************************* %>
<style type="text/css">
    ...
    <table border=0 cellpadding=0 width=100% cellspacing=1 bgcolor="#CCCCCC">
      ...
      <tr> 
        <td height="25" colspan="4" align=center bgcolor=#F8F9FD>具体内容：<BR>
		   <textarea style="display:none;" name="Remark"><%=Server.HtmlEncode(trim(n_Body)&" ")%>
		   </textarea>
      ...
      <tr> 
        ...
        &nbsp;(填URL)
        <input type=button value="上传图片..." onClick="showUploadDialog('image', 'add.pic', '')"></td>
      </tr>
      <tr> 
